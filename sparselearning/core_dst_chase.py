...
from sparselearning.norm_funcs import *
...
def add_sparse_args(parser):
    ...
    parser.add_argument('--ws_wise', action='store_true', help='mag_wise')
    parser.add_argument('--ws_decay', type=str, default='', help='weight & spike score mix types. Choose from: stepsigmoid, only1.')
    parser.add_argument('--tau_switch_ratio', type=float, default=0.3, help='stepsigmoid switch ratio')
    parser.add_argument('--score_norm', type=str, default='', help='weight & spike scores norm funcs. Choose from: minmax, zscore, rank, or none.')
...
# Modify decay classes to expose parameters controlling decay rate



class StepSigmoidDecay:
    def __init__(self, death_rate, T_max,
                 eta_min=0.0, k=0.2, center_ratio=0.5, active_ratio=0.3):
        ...
        self.active_T = int(T_max * active_ratio)  # Sigmoid 有效范围
        self.t0 = self.active_T * center_ratio     # Sigmoid 中心点
        self.epoch = 0
        self.lambda_val = self._compute_lambda()

    def _compute_lambda(self):
        # 超过 active_T 后恒定为 0
        if self.epoch >= self.active_T:
            return 0.0
        # Sigmoid 平滑衰减
        return self.death_rate - (self.death_rate - self.eta_min) / (
            1 + np.exp(-self.k * (self.epoch - self.t0))
        )
...
class Masking(object):
    def __init__(self, optimizer, death_rate=0.3, growth_death_ratio=1.0, death_rate_decay=None, death_mode='magnitude', growth_mode='gradient', redistribution_mode='none', args=None, train_loader=None, device=None):
        ...
        if self.args.ws_decay == 'stepsigmoid':
            self.ws_decay = StepSigmoidDecay(1.0, math.floor((self.args.stop_gmp_epochs)*len(self.train_loader)), active_ratio=self.args.tau_switch_ratio)
            print("use stepsigmoid, switch ratio use", self.args.tau_switch_ratio)
        elif self.args.ws_decay == 'only1':
            self.ws_decay = Only1Decay(1.0, math.floor((self.args.stop_gmp_epochs)*len(self.train_loader)))
        else:
            raise Exception("no ws_decay chosen")

        self.scoreNormalizer = ScoreNormalizer(self.args.score_norm)
    ...
    def get_module(self,key):
        ...
        if self.module.__class__.__name__ =="ResNet" \
            or self.module.__class__.__name__ =="ResNetbasic" \
            or self.module.__class__.__name__ =="Model" \
            or self.module.__class__.__name__ =="PreActSEWResNet":
            return getattr(getattr(self.module, key[0])[key[1]],key[2])
        if self.module.__class__.__name__ =="SEWResNet" or self.module.__class__.__name__ =="ResNetbasic" or self.module.__class__.__name__ =="Model":
    ...
    def merge_filters (self):
        ...
        for active_grow_key in self.module.layer2split:
            ...
            tmp = self.test_return_filter_mask(self.get_mask_name(active_grow_key))
            if tmp.sum() < tmp.numel():
                del_mask = tmp.detach().cpu()
    ...
    def prune_score(self,prune_layer_index,total_to_prune):
        ...
        for index in prune_layer_index:
            ...
            if self.args.ws_wise:
                if "VGG" in self.module.__class__.__name__ :
                    mem_name = "features."+str(self.module.next_layers_LIF[index])
                elif "PreActSEWResNet" in self.module.__class__.__name__:
                    mem_name = '.'.join(map(str, self.module.next_layers_LIF[index]))
                else:
                    raise Exception('unknown model in prune_score')

                if self.mem_monitor.name_records_index_pre[mem_name] != None:
                    mem = self.mem_monitor.name_records_index_pre[mem_name][filter_mask].detach().cpu().numpy().tolist()
                else:
                    mem = self.mem_monitor.name_records_index[mem_name][filter_mask].sum(dim=(1,2)).detach().cpu().numpy().tolist()
 
                all_scores_mem += mem
                pass 
            else :
                raise Exception('layer prune metric illegal!')
            ...
            for filter_grad, filter_mask in zip(grad,mask):
                ...
                if self.args.ws_wise:
                    ...
                    all_scores_umm.append(grad_magnitude)
                else :
                    raise Exception('layer prune metric illegal!')
               
        print ("current score lengh",len(all_scores_umm))
        # print ("all_scores",all_scores)
        all_scores = self.scoreNormalizer.blend(all_scores_mem, all_scores_umm, self.ws_decay.get_dr())


        try:
            acceptable_score = np.sort(np.array(all_scores))[-int(len(all_scores)-total_to_prune)]
        except IndexError:
            print('len(prune_layer_index)', len(prune_layer_index), 'no layer to be pruned, no all_scores')
            acceptable_score = 0.
                    ...
                    for index in self.module.layer2split:
                        ...
                        if index not in residual_layer.keys():
                            ...
                            if self.args.ws_wise:
                                ...
                                if self.mem_monitor.name_records_index_pre[mem_name] != None:
                                    mem = self.mem_monitor.name_records_index_pre[mem_name].detach().cpu().numpy().tolist()
                                else:
                                    mem = self.mem_monitor.name_records_index[mem_name].sum(dim=(1,2)).detach().cpu().numpy().tolist()
                            ...
                            for filter_grad,filter_mask,filter_name in zip(grad, mask,self.filter_names[self.get_mask_name(index)]):
                                ...
                                if filter_name==1.0:
                                    ...
                                    if self.args.ws_wise:
                                        grad_magnitude = torch.abs(filter_grad) .mean().item()
                                        grad_magnitude = self.scoreNormalizer.blend2(mem[filter_index], grad_magnitude, self.ws_decay.get_dr())
                                    else :
                                        raise Exception('layer prune metric illegal!')
    ...
    def step(self):
        ...
        if self.args.sparse:
            ...
            if self.args.filter_dst:
                if  self.steps >= self.initial_prune_time and self.steps < self.final_prune_time :
                    if (self.steps+ (1000/2)) % self.args.layer_interval== 0 :
                        ...
                        print ('===========del layer===============')
                        print('ws_decay coeffi', self.ws_decay.get_dr())
                        self.del_layer()
            ...
            if self.args.mest or self.args.dst:
                if self.prune_every_k_steps is not None:
                    if (self.steps % self.prune_every_k_steps == 0):
                    # cal grad change            
                        if len(self.grad_pre.keys()) == 0:
                            self.update_grad_pre()
                        else: 
                            self.compare_grad()
                            self.update_grad_pre()
    ...
    def update_grad_pre(self):
        # 忘记了，对比的是权重
        for module in self.modules:
            for name, weight in module.named_parameters():
                if name not in self.masks: continue
                self.grad_pre[name] = weight.clone()
    ...
    def compare_grad(self):
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
               if name not in self.masks: continue
               # 对一个有原始值的mask Tensor，同一个尺寸的grad和delta_grad，如果delta_grad里面，某个元素为0，且对应位置的grad是负值，则对mask同一位置置零
               grad = weight.clone()
               delta_grad = self.grad_pre[name] - grad
               grads.append(grad)
               delta_grads.append(delta_grad)
               cond1 = (delta_grad == 0.)
               cond2 = (grad < 0.)
               mask_cond = cond1 & cond2
               self.masks[name][mask_cond] = 0
        # Gather all scores in a single vector and normalise
        grads = torch.cat([torch.flatten(x) for x in grads])
        scores = torch.cat([torch.flatten(x) for x in delta_grad])
        zero_count = (scores==0.).sum().item()
        indexs = torch.where(scores==0.)
        neg_grads = grads[indexs]
        if zero_count:
            print("################## weight change zero count", zero_count)
            print("################## no change neg weight", len(indexs))
        self.apply_mask()
    ...
    def cal_UMM(self):
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                if name not in self.masks: continue
                left_indexs = list(range(len(weight.shape)))[1:]
                # ML0 = (weight != 0).sum(dim=left_indexs)
                ML0 = weight.numel()
                WSUM = weight.abs().sum(dim=left_indexs)
                UMMs[name] = WSUM/(ML0+1e-6)
        return UMMs
    
    def test_return_filter_mask(self, key):
        for module in self.modules:
            for name, weight in module.named_parameters():
                if name == key:
                    left_indexs = list(range(len(weight.shape)))[1:]
    ...
    def add_memMonitor(self, monitor):
        self.mem_monitor = monitor
