...
from models.vggsnnTAB import TABatchNorm3d
...
def get_cifar100_random_loaders(args, ratio=0.7):
    """Creates augmented train, validation, and test data loaders."""
    cifar_mean = (0.5070751592371323, 0.48654887331495095, 0.4409178433670343)
    cifar_std = (0.2673342858792401, 0.2564384629170883, 0.27615047132568404)
    # Data
    print('==> Preparing random dataloader at',str(ratio))
    ...
    # get random train_loader
    trainset=Random_DATA_Handler(trainset,ratio)
    print('==> Created at ratio',str(ratio))
...
class Random_DATA_Handler(Dataset):
    def __init__(self, parent_dataset,ratio):
        # orginal data
        data=parent_dataset.data
        targets=parent_dataset.targets
        
        # sample
        tmp_idxs = np.arange(len(parent_dataset))
        np.random.shuffle(tmp_idxs)
        tmp_idxs=tmp_idxs[:int(len(parent_dataset)*ratio)]
        
        
        self.X = parent_dataset.data[tmp_idxs]
        self.Y = list(np.array(parent_dataset.targets)[tmp_idxs])
...
def get_cifar100_dataloaders(args, validation_split=0.0, max_threads=10):
    ...
    trainset = torchvision.datasets.CIFAR100(root='/datasets2', train=True, download=True,
                                             transform=transform_train)
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=2)

    testset = torchvision.datasets.CIFAR100(root='/datasets2', train=False, download=True, transform=transform_test)
...
def get_cifar10_dataloaders(args, validation_split=0.0, max_threads=10):
    ...
    full_dataset = datasets.CIFAR10('/datasets2', True, train_transform, download=True)
    test_dataset = datasets.CIFAR10('/datasets2', False, test_transform, download=False)
...
def split_weights(net):
    ...
    for m in net.modules():
        if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
            ...
            if isinstance(m, TABatchNorm3d):
                for sub_m in m.modules():
                    if isinstance(sub_m, TABatchNorm3d):
                        continue
                    if hasattr(sub_m, 'weight'):
                        no_decay.append(sub_m.weight)
                    if hasattr(sub_m, 'bias'):
                        no_decay.append(sub_m.bias)
