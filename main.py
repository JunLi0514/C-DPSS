# 和chaseSNN相比，变化出现在train mask的step上。chaseSNN 在mask为None时启用mask.optim.step()，这个代码逻辑上非常没必要
# train里面的mask设置的是None, 修改回原来的mask step
...
import re
import random
from models.Monitor import SpikeMonitor
...
from sparselearning.utils import get_mnist_dataloaders, get_cifar10_dataloaders, plot_class_feature_histograms, get_cifar100_dataloaders, split_weights
...
# from models.vggsnn import vgg16_bn
from models.vggsnnTbn import vgg16_bn
from models.vggsnnTAB import vgg16_bn as vggsnnTAB
from models.sew_resnet import sew_resnet18, sewBatchNorm3d
from models.preact_sewresnet import preact_sew_resnet18
...
def train(args, model, device, train_loader, epoch,mask=None, optimizer=None):
    model.train()
    train_loss = 0
    correct = 0
    n = 0
    num_iters = len(train_loader)
    mask.mem_monitor.reset()
    mask.mem_monitor.enable()
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss = 0.0
        train_loss = 0.0
        pred = torch.zeros_like(output_list[0])
        for output in output_list:
           loss += criterion(output, target) / args.timestep
           train_loss += loss.item()
           pred += output
    ...
    mask.mem_monitor.div_channels()
    mask.mem_monitor.disable()
...
def main():
    ...
    parser.add_argument('--timestep', type=int, default=2, help='timestep for SNN')
    ...
    for i in range(args.iters):
        ...
        elif args.model == 'vgg16snn':
            model = vgg16_bn(num_classes=outputs, total_timestep=args.timestep,store_v_seq=True).to(device)
        elif args.model == 'sew_resnet18':
            model = preact_sew_resnet18(num_classes=outputs, T=args.timestep).to(device)
        elif args.model == 'vggsnnTAB':
            model = vggsnnTAB(num_classes=outputs, total_timestep=args.timestep).to(device)
        ...
        if args.optimizer == 'sgd':
            ...
            # optimizer = optim.SGD(model.parameters(),lr=args.lr,momentum=args.momentum,dampening=args.l2)
            optimizer = optim.SGD(parameters,lr=args.lr,momentum=args.momentum,weight_decay=args.l2)
        ...
        mask.add_memMonitor(SpikeMonitor(mask.module, 
                            args.timestep,
                            train_loader.dataset.__len__()))

        if args.mask_wise:
            channel_prune_type = 'mask_wise'
        elif args.mag_wise:
            channel_prune_type = 'mag_wise'
        elif args.kernal_wise:
            channel_prune_type = 'kernal_wise'
        elif args.connection_wise:    
            channel_prune_type = 'connection_wise'
        elif args.ws_wise:    
            channel_prune_type = 'ws_wise' + ('_%s_'%args.ws_decay)+('_%s'%args.score_norm)
        tmp = ""
        if not args.filter_dst:
            tmp += "_nofilterPrune_"
        save_dir = "./Chase_models/saved_models/" \
            +args.data+'/'\
            +str(args.indicate_method)  \
            + 'model_' +tmp+args.model \
            +str(args.seed) +'/' \
            + '/density_' + str(args.density) + '/stop_gmp_epochs_' + str(args.stop_gmp_epochs)   \
            + '/epoch_' + str(args.epochs) +  '/layer_interval_' + str(args.layer_interval)\
            + '/start_layer_rate_' + str(args.start_layer_rate)\
            + '/'+channel_prune_type
        ...
        }, filename=os.path.join(save_dir, 'premodel_ckpt_init.pth'))
        ...
        best_acc=0
        for epoch in range(epoch_start, pretrain_epoch):

            t0 = time.time()

            # train(args, model, device, train_loader, epoch,mask)
            train(args, model, device, train_loader, epoch, mask=mask, optimizer=mask.optimizer)
            ...
            save_best=True
            if epoch%20==0 or epoch==(pretrain_epoch-1):
                save_checkpoint({
                   'epoch': epoch,
                   'state_dict': model.state_dict(),
                   'optimizer': optimizer.state_dict(),
                }, filename=os.path.join(save_dir, 'premodel_ckpt%d.pth'%epoch))
